select s1.group_id,case when s1.group_name is null then 'No group' else s1.group_name end as group_name ,users.id user_id,users.name,users.email,s1.check_in_created_at,s1.check_in_address,s1.check_in_latitude,s1.check_in_longitude
...
from check_outs a
inner join locations b on a.user_id=b.user_id and a.created_at=b.created_at
) as s2 on s1.user_id=s2.user_id and s1.check_in_created_at=s2.check_out_created_at

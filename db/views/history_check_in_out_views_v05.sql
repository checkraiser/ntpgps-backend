select groups.id group_id, case when groups.name is null then 'No group' else groups.name end as group_name,users.id user_id,users.name,users.email,a.created_at check_in_created_at,a.address check_in_address,a.percentage check_in_percentage,b.created_at check_out_created_at,b.address check_out_address,b.percentage check_out_percentage
from check_ins a
